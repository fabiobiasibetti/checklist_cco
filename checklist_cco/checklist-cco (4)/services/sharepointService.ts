...
function resolveFieldName(mapping: Record<string, string>, target: string): string {
  ...
  // Fallback especial para campos comuns do SharePoint
...
export const SharePointService = {
  ...
  async updateDeparture(token: string, departure: RouteDeparture): Promise<string> {
    ...
    // Mapeamento explícito de campos baseado na solicitação do usuário
    const raw: any = {
    ...
    Object.keys(raw).forEach(k => {
      ...
      // Title é sempre permitido. Outros apenas se existirem na lista e não forem somente leitura.
      if (int === 'Title' || (internalNames.has(int) && !readOnly.has(int))) {
          fields[int] = raw[k];
      }
    });

    // Se o ID for numérico e existir, é uma atualização (PATCH)
    const isUpdate = departure.id && !isNaN(Number(departure.id)) && departure.id !== '';
