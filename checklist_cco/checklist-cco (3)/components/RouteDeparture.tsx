...
const RouteDepartureView: React.FC = () => {
  ...
  const calculateGap = (inicio: string, saida: string): { gap: string, status: string } => {
    ...
    // Se a diferença for positiva, é atraso. Se negativa, adiantado (OK).
    ...
    if (confirm('Deseja excluir esta rota permanentemente do SharePoint?')) {
      ...
      try {
        ...
        setRoutes(routes.filter(r => r.id !== id));
          ...
          alert("Erro ao excluir no SharePoint");
      } finally {
          setIsSyncing(false);
      }
    }
  ...
  const updateCell = async (id: string, field: keyof RouteDeparture, value: string) => {
    ...
    // Auto-calculates Gap and StatusOp if inicio or saida changes
    ...
    // Se mudar a data, atualiza a semana automaticamente
    if (field === 'data') {
    ...
    try {
      ...
      console.error("Erro na sincronização parcial");
    } finally {
      setIsSyncing(false);
    }
  };
      ...
      <div className="flex justify-between items-center mb-4 shrink-0">
        <div>
          <h2 className="text-xl font-bold text-gray-800 dark:text-white flex items-center gap-2">
            ...
            Saída de Rotas (SharePoint)
            {isSyncing && <Loader2 size={16} className="animate-spin text-blue-500 ml-2"/>}
          </h2>
          <p className="text-xs text-gray-500 dark:text-gray-400">Controle de Horários & Gap Automático</p>
        </div>
      ...
      <div className="flex-1 overflow-auto bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm relative scrollbar-thin">
        <table className={`w-full border-collapse text-[10px] ${isFullscreen ? 'min-w-full' : 'min-w-[1500px]'}`}>
          <thead className="sticky top-0 z-20 bg-blue-900 text-white uppercase font-bold tracking-wider text-center">
            <tr>
              <th className="p-2 border border-blue-800 w-20">Semana</th>
              <th className="p-2 border border-blue-800 w-24">Rota</th>
              <th className="p-2 border border-blue-800 w-28">Data</th>
              <th className="p-2 border border-blue-800 w-24">Início</th>
              <th className="p-2 border border-blue-800 min-w-[150px]">Motorista</th>
              <th className="p-2 border border-blue-800 w-24">Placa</th>
              <th className="p-2 border border-blue-800 w-24">Saída</th>
              <th className="p-2 border border-blue-800 w-32">Motivo</th>
              <th className="p-2 border border-blue-800 min-w-[200px]">Obs</th>
              <th className="p-2 border border-blue-800 w-16">Geral</th>
              <th className="p-2 border border-blue-800 w-16">Av</th>
              <th className="p-2 border border-blue-800 w-40">Operação</th>
              <th className="p-2 border border-blue-800 w-24">Status Op</th>
              <th className="p-2 border border-blue-800 w-24">Tempo</th>
          ...
          <tbody>
            ...
            {routes.map((route) => (
              <tr key={route.id} className={`hover:bg-blue-50/50 dark:hover:bg-blue-900/10 border-b ${getRowStyle(route)}`}>
                ...
                <td className="p-0 border border-gray-200 dark:border-slate-800 text-center">
                  <div className={`w-full h-full flex items-center justify-center font-black ${getStatusBadgeStyle(route.statusOp)}`}>
                    {route.statusOp}
                  </div>
                </td>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            <div className="bg-blue-900 text-white p-4 flex justify-between items-center">
                <h3 className="font-bold">Nova Saída</h3>
                <button onClick={closeModal}><X size={24} /></button>
            </div>
            <form onSubmit={handleSubmit} className="p-6 space-y-4 bg-gray-50 dark:bg-slate-900">
                <div className="grid grid-cols-2 gap-4">
                    <input type="date" required value={formData.data} onChange={e => setFormData({...formData, data: e.target.value})} className="p-2 border dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-sm"/>
                    <input type="text" required placeholder="ROTA" value={formData.rota} onChange={e => setFormData({...formData, rota: e.target.value.toUpperCase()})} className="p-2 border dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-sm font-bold"/>
                </div>
                <input type="text" required placeholder="OPERAÇÃO / CLIENTE" value={formData.operacao} onChange={e => setFormData({...formData, operacao: e.target.value.toUpperCase()})} className="w-full p-2 border dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-sm font-bold"/>
                <div className="grid grid-cols-2 gap-4">
                    <input type="text" required placeholder="MOTORISTA" value={formData.motorista} onChange={e => setFormData({...formData, motorista: e.target.value.toUpperCase()})} className="p-2 border dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-sm"/>
                    <input type="text" required placeholder="PLACA" value={formData.placa} onChange={e => setFormData({...formData, placa: e.target.value.toUpperCase()})} className="p-2 border dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-sm font-bold"/>
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <input type="text" required placeholder="INÍCIO (00:00:00)" value={formData.inicio} onChange={e => setFormData({...formData, inicio: e.target.value})} className="p-2 border dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-sm font-mono"/>
                    <input type="text" placeholder="SAÍDA (00:00:00)" value={formData.saida} onChange={e => setFormData({...formData, saida: e.target.value})} className="p-2 border dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-sm font-mono"/>
                </div>
                <button type="submit" disabled={isSyncing} className="w-full py-3 bg-blue-600 text-white font-bold rounded-xl flex items-center justify-center gap-2">
